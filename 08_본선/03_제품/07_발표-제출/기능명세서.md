...
> **코드 SSOT**: `_vendor/JB_project2/app/cclConsole.core.js`·`cclConsole.data.js`. 히어로 = **CCL-0001**(전주 카페 운영자 운전자금, `BIZ-REF-0001`).
...
> **미검증 주의**: 서버 레이어(②~⑤)는 현재 브라우저 `localStorage` 재현이며 서버 승격은 `[TBD/E1]`. MCI/EAI 전문 스키마·Nexacro WebView 임베딩은 `[미검증]`(비공개 규격). 에이전트 수 정합(CCL 도메인 8 ↔ 전체 콘솔 14 목표) 통합 명명은 `[Open Question]`.
...
| 1.3.1 | AuditEvent 공통필드·해시체인(previousHash) | ✅ | E4 | `auditChainRecords` |
...
**골든패스 9스텝**(CCL-0001): 로그인 → 케이스보드 → 케이스 생성(`received`) → 상세(`collecting`) → 에이전트 실행뷰(`aiReview`) → 승인대기함(`humanReview`) → 승인/거부/수정후승인 → 알림(3중 게이트) → 감사 봉인(`doneHold`)(09_flow §0, [E4]).
...
| F-04 | 에이전트 세트 | canon §2 메인 14종 | 메인 14 + 확장 하네스별 자체 세트(corporate-credit 8 + 전세보호 10 + FDS 8 + JBWC 13 = 39 신규 id) | 레이어 추가 | E4 |
