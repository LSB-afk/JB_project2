...
| 페이즈 | 범위 | 의존 | 잠정 기간 |
|---|---|---|---|
| P0 정의-합의 | 정의서·11블록·미결 결정 합의 | (루트) | 본선 당일 |
| P1 데이터-연동기반 | 공공/내부 데이터·API·라이선스·코어뱅킹 연동 설계 | P0 | 2~4주 |
| P2 에이전트-스킬-메모리 | 판단→행동→검증 로스터·Case/AgentRun·정책엔진·고객메모리·시뮬 | P1 | 4~8주 |
| P3 보안-거버넌스 | PII 비반출·토큰화·반출스캔·망분리·승인루프·감사 | P1 | 파일럿 계약 후 산정 |
| P4 UI-조직도-콘솔 | 조직도 메인 UI·JB 웹 디자인 차용·RM 콘솔·신뢰보정 UX | P0 | 1~2개월 |
| P5 통합-로컬모델-시연 | 로컬모델 시연·골든패스 3종·e2e·OSS 스택 | P2·P3·P4 | 1~2개월 |
| P6 본선-리허설 | 발표·시연 완주·폴백·리허설 | P5 | 본선 당일 |

> **기간 주(注)**: 잠정 기간은 팀 산정 잠정치이며 파일럿 계약 시 정교화한다 [E1]. P0·P6은 본선(2026-07-05) 당일 완결분, P1~P5는 본선 후 양산·서버 승격 기준의 추정 소요다(P3 망분리·내부망 연동은 은행 내부망 접근이 전제라 계약 후에만 산정 가능).
