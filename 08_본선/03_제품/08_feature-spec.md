...
## 0. 히어로 케이스 ID 정합 — [Open Question]

- 본 작업 지시 SSOT는 본선 히어로를 **CCL-0001(전주 카페 운전자금)** 으로 지정한다. 반면 선행 Wave1 문서(PRD·agent-roster·키스톤)는 예선 코드 **JBG-104(전주 중앙로 카페, riskScore 88)** 를 사용한다.
- 두 ID는 **동일 히어로(전주 카페 소상공인 여신)** 를 가리키는 것으로 보이나 코드가 다르다. 이 문서는 지시 SSOT를 따라 **CCL-0001** 을 정본으로 쓰되, 아래 표기 규약을 둔다.
- 규약: `CCL-0001 (구 JBG-104)`. **[Open Question — 7/4 팀 확정 필요]**: 두 계열사·다도메인 확장 시 케이스 코드 네이밍(`{계열사}{도메인}-{seq}`?)을 `rules/naming-rules.md`에서 잠글 것. 확정 전까지 데모 스크립트·코드·문서 간 ID 불일치 리스크 존재 → **[미검증]**.
...
**데모가능 요약**: ✅실동작 12 / 🟡부분 8 / ⛔비데모 4 (총 24 항목). E4(작동 검증) 목표 달성 = ✅ 12항목 중 결정형 로직. 라이브 LLM 스트리밍(2.3.1)·PII 반출스캔 E4 격상은 **[목표/7-4]** — 현재 미완(키스톤 §정직한 전제).
...
| 항목 | 등급 | 처리 |
|---|---|---|
| 히어로 ID CCL-0001 vs JBG-104 | [Open Question] | §0, 7/4 팀·naming-rules 확정 |
