...
test("위험 접수 보드가 lifecycle 컬럼과 카드 필수 정보를 표시한다", async ({ page }) => {
  ...
  const card = page.locator('[data-board-column="humanReview"] .jpo-card', { hasText: "전세위험-0004" });
  await expect(card).toContainText("익명 고객 0004");
  ...
  await expect(page.locator('[data-board-column="received"]')).toContainText("전세위험-0006");
  await expect(page.locator('[data-board-column="externalLinked"]')).toContainText("전세위험-0005");
...
test("route가 새로고침 후에도 유지된다 (v2 메뉴)", async ({ page }) => {
  ...
  await expect(page.locator(".jbwc-detail-panel")).toContainText("전세위험-0002");
  await page.reload();
  await expect(page.locator(".jbwc-detail-panel")).toContainText("전세위험-0002");
});
...
test("모든 count는 role scope query에서 오고 타 역할 seed는 노출되지 않는다", async ({ page }) => {
  ...
  await expect(page.locator("#jpo-search-results .jbwc-search-hit").first()).toContainText("전세위험-0001");
  await page.locator("#jpo-search-results .jbwc-search-hit").first().click();
  await expect(page.locator(".jbwc-detail-panel")).toContainText("전세위험-0001");
...
test("경·공매 접수: 스냅샷·위험 신호·체크·감사·승인 기록과 high 이상 human review", async ({ page }) => {
  ...
  await expect(page.locator("#jpo-new-case-form")).toContainText("담당자 검토 필요");
  ...
  const newCaseLabel = newCaseId.replace("JEONSE-", "전세위험-");
  await expect(page.locator(".jbwc-detail-panel")).toContainText(newCaseLabel);
  ...
  await expect(page.locator("#page-content")).toContainText(newCaseLabel);
});

test("API 키 없는 기본 모드: 스냅샷/fallback 표시가 화면에 노출된다", async ({ page }) => {
  await page.goto("/index.html#/roles/jeonse-protection/price-enrich");
  await expect(page.locator(".jbwc-breadcrumb")).toContainText("실시간 API 미연결 · 저장 기준 사용");
  await expect(page.locator("#page-content")).toContainText("실시간 API 미연결");
  ...
  const loop = await page.evaluate((key) => {
    const db = JSON.parse(window.localStorage.getItem(key));
    return {
      enrichedAudit: db.jeonse_audit_logs.some((a) => a.action === "DATA_ENRICHED"),
      evidence: db.jeonse_evidence.some((e) => e.kind === "price" && e.title.includes("시세 데이터 보강")),
      evaluator: db.jeonse_audit_logs.some((a) => a.action === "EVALUATOR_CHECKED" && a.targetType === "enrichment"),
    };
  }, JPO_DB_KEY);
  expect(loop.enrichedAudit && loop.evidence && loop.evaluator).toBe(true);
});
...
test("담당자 업무 언어: 내부 개발 용어가 화면에 노출되지 않는다", async ({ page }) => {
  const forbidden = ["CUST-JS", "JEONSE-SNAP", "sourceMode", "fallback", "snapshot", "스냅샷", "중앙값", "role scope"];
  for (const route of ["/roles/jeonse-protection/board", "/roles/jeonse-protection/cases", "/roles/jeonse-protection/price-enrich", "/roles/jeonse-protection/sale-comparables"]) {
    await page.goto(`/index.html#${route}`);
    await expect(page.locator("#page-content")).not.toBeEmpty();
    const text = await page.locator("#page-content").innerText();
    for (const term of forbidden) {
      expect(text.includes(term), `${route} 화면에 내부 용어 '${term}' 노출`).toBe(false);
    }
    expect(/JEONSE-\d{4}/.test(text), `${route} 화면에 내부 케이스 ID 원문 노출`).toBe(false);
  }
  await expect(page.locator("#page-content")).toContainText("시세 비교 기록");
  await page.goto("/index.html#/roles/jeonse-protection/board");
  await expect(page.locator("#page-content")).toContainText("전세보호 업무 기준 집계");
});

test("상세 패널 6섹션 → 상세 페이지 탭 8 → 근거 무결성 → route 유지", async ({ page }) => {
  await page.goto("/index.html#/roles/jeonse-protection/board");
  await page.locator(".jpo-card", { hasText: "전세위험-0001" }).first().click();
  const panel = page.locator(".jpo-case-panel");
  for (const section of ["1. 기본 식별", "2. 분석 결과", "3. 담당 AI 업무지원", "4. 검토/승인 정책", "5. 근거 피드", "6. 감사 로그"]) {
    await expect(panel).toContainText(section);
  }
  await expect(panel).toContainText("전세사기 여부에 대한 확정 판정이 아닙니다");
  await expect(panel).toContainText("시세 비교 Agent");
  await expect(panel).toContainText("검증 Agent");

  await panel.locator("[data-jpo-open-full]").click();
  await expect(page).toHaveURL(/\/roles\/jeonse-protection\/cases\/JEONSE-0001\/full/);
  for (const tab of ["개요", "위험 신호", "시세·실거래", "권리관계", "보증·HUG", "피해지원", "AI 분석", "근거·감사"]) {
    await expect(page.locator(".jpo-case-tabs")).toContainText(tab);
  }
  await page.locator('[data-jpo-case-tab="market"]').click();
  await expect(page.locator("#page-content")).toContainText("인근 매매 거래 기준가");
  await page.locator('[data-jpo-case-tab="evidence"]').click();
  await expect(page.locator("#page-content")).toContainText("근거 피드");
  await page.locator("[data-jpo-verify-evidence]").click();
  await expect(page.locator("[data-evidence-check]")).toContainText("무결성 확인 완료");
  const selected = await page.evaluate((key) => {
    const db = JSON.parse(window.localStorage.getItem(key));
    return db.jeonse_audit_logs.some((a) => a.action === "CASE_SELECTED" && a.targetId === "JEONSE-0001");
  }, JPO_DB_KEY);
  expect(selected).toBe(true);
  await page.reload(); // initScript가 DB를 재시드하지만 route/탭은 유지되어야 한다
  await expect(page.locator(".jpo-case-tabs")).toContainText("근거·감사");
  await expect(page.locator("#page-content")).toContainText("전세위험-0001");
});

test("루프 검증(Evaluator 분리)·자동화 준비: EVALUATOR_CHECKED와 감지 전용 패널", async ({ page }) => {
  await page.goto("/index.html#/roles/jeonse-protection/agent-harness");
  await expect(page.locator("#page-content")).toContainText("루프 자동화 준비");
  await expect(page.locator("#page-content")).toContainText("자동 실행 준비됨");
  await expect(page.locator("#page-content")).toContainText("자동 종결·자동 승인은 금지");

  await page.getByRole("button", { name: /경매 기일 통지가 왔어/ }).click();
  await expect(page.locator("#page-content")).toContainText("루프 검증(분리 실행)");
  const evaluator = await page.evaluate((key) => {
    const db = JSON.parse(window.localStorage.getItem(key));
    const run = db.jeonse_agent_runs.find((r) => r.agentId === "jpo-evaluator");
    return {
      run: Boolean(run),
      audit: db.jeonse_audit_logs.some((a) => a.action === "EVALUATOR_CHECKED" && a.actorId === "jpo-evaluator"),
      evidence: db.jeonse_evidence.some((e) => e.kind === "evaluator"),
      humanForced: run && run.requiresHumanReview === true,
    };
  }, JPO_DB_KEY);
  expect(evaluator.run && evaluator.audit && evaluator.evidence).toBe(true);
  expect(evaluator.humanForced).toBe(true);

  await page.getByRole("button", { name: "/jeonse-daily-triage · 일일 분류 점검(준비)" }).click();
  await expect(page.getByRole("status")).toContainText("일일 분류 점검(감지 전용)");
});
