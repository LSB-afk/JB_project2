...
test("자체 검증 루프 + 보안 훅 차단(PII 접수/자동 종결/사람 승인)", async ({ page }) => {
  ...
  // 등록 하네스마다 6개 검증기 — 하네스가 추가되어도 견고하도록 배수/최소치로 확인(하드코딩 개수 회귀 방지)
  expect(selfTest.results.length).toBeGreaterThanOrEqual(24);
  expect(selfTest.results.length % 6).toBe(0);
